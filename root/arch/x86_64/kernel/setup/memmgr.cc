...
void* memmgr::memmgr_alloc(std::size_t size, std::size_t align)
{
	...
	for (memmap_entry* ent = mm->free_chain.get_head();
		...
		if (ent->bytes == size) {
			...
			memmap_entry* newent = mm->new_entry();
			if (newent == 0)
				continue;

			if (align_gap != 0) {
				memmap_entry* newent2 = mm->new_entry();
				if (newent2 == 0) {
					newent->unset();
					continue;
				}
...
void memmgr::memmgr_free(void* p)
{
	...
	for (memmap_entry* ent2 = free_chain.get_head();
	     ent2;
