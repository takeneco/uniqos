...
public:
	...
	void add_free(uptr head, uptr bytes, bool avoid);

	void* mem_alloc(uptr size, uptr align, bool kern_forget);
	void mem_free(void* p);

	bool reserve_range(uptr head, uptr bytes);
...
private:
	struct entry
	{
		...
		void set(uptr _head, uptr _bytes) {
			head = _head;
			bytes = _bytes;
		}
		void unset() {
...
private:
	entry* new_entry(uptr head, uptr bytes);
...
void mem::add_free(uptr head, uptr bytes, bool avoid)
{
	if (bytes == 0)
		return;

	entry* ent = new_entry(head, bytes);
	if (ent == 0)
		return;
...
void* mem::mem_alloc(uptr size, uptr align, bool kern_forget)
{
	entry* ent;
	...
	if (kern_forget) {
		...
		if (!ent)
			ent = _mem_alloc(free_chain, nofree_chain, size, align);
	...
	return reinterpret_cast<void*>(ent->head);
}
...
/// この関数で設定しなければならない。
bool mem::reserve_range(uptr head, uptr bytes)
{
	return
	    _reserve_range(free_chain, nofree_chain, head, bytes) &&
	    _reserve_range(avoid_free_chain, avoid_nofree_chain, head, bytes);
}
...
mem::entry* mem::new_entry(uptr head, uptr bytes)
{
...
/// @param[in] _nofree_chain 使用メモリチェイン。
/// @param[in] r_head 取り除くメモリの先頭アドレス。
/// @param[in] r_tail 取り除くメモリの終端アドレス。
//
/// _free_chain から取り除いたメモリは _nofree_chain に格納する。
bool mem::_reserve_range(
    entry_chain& _free_chain, entry_chain& _nofree_chain,
    uptr head, uptr bytes)
{
	uptr r_head = head;
	uptr r_tail = head + bytes;
	     ...
	     ent = _free_chain.next(ent))
	{
		...
		if (e_head < r_head && r_tail < e_tail) {
			entry* nofree = new_entry(head, bytes);
			...
			_nofree_chain.insert_head(nofree);
		...
		if (r_head <= e_tail && e_tail < r_tail) {
			...
			entry* nofree = new_entry(h, e_tail - h);
			if (!nofree)
				return false;
			_nofree_chain.insert_head(nofree);
    ...
    uptr size, uptr align)
{
	...
	{
		align_gap = up_align(ent->head, align) - ent->head;
	...
	if (ent->bytes == size) {
		...
		if (align_gap != 0) {
			entry* newent = new_entry(ent->head, align_gap);
			if (!newent)
				return 0;
		...
		r = new_entry(ent->head, size);
		if (!r)
			return 0;

		_nofree_chain.insert_head(r);
    ...
    void* p)
{
	...
	uptr head = reinterpret_cast<uptr>(p);
	entry* ent;
...
{
	get_mem()->add_free(head, bytes, avoid);
}
