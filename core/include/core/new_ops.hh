...
public:
    struct interfaces
    {
        ...
        using AllocateIF = cause::pair<void*> (*)(mem_allocator* x, uptr bytes);
        AllocateIF Allocate;

        using DeallocateIF = cause::t (*)(mem_allocator* x, void* p);
        DeallocateIF Deallocate;

        using GetSizeIF =cause::pair<uptr> (*)(mem_allocator* x, void* p);
        GetSizeIF GetSize;
    };
...
class mempool;
void* operator new (uptr size, mempool*);   // no implements, use mem_allocator.
void* operator new[] (uptr size, mempool*); // no implements, use mem_allocator.
